...
main:
    Target_bed = split_bed(Negative_bed)
  ...
  target_BA = Basic_Annotation( Scored.scored.combine(Negative_bed ), editors )
  target_A = annotate( target_BA.VCF.flatten().combine(Negative_bed) )
  //reunify
  Combine_fail = combine_failed(target_crispr.failed.collect(), 'Negative_Control_Library')
  Combine_csv = combine_general(target_BA.CSV.collect(), 'Negative_Control_Library')
  vcfs = combine_vcfs(    target_BA.VCF.flatten().map( file -> [ file.getBaseName().tokenize('_')[4], file ]).groupTuple(by: [0]), 'Negative_Control_Library', Combine_csv.ID_dic)
  vep = combine_annotations(target_A.Annotations.flatten().map( file -> [file.getBaseName().tokenize('_')[4], file]).groupTuple(by: [0]), 'Negative_Control_Library', Combine_csv.ID_dic)
  outputCSV=Combine_csv.csv
  outputVEP=vep.tsv
  output=to_excel(Combine_csv.csv, vep.tsv.collect(), 'Negative_Control_Library.xlsx')
